...
void systems::Collision(flecs::world& world) {
   // PreUpdate systems: Detect collisions and dispatch events
   // PostUpdate systems: Cleanup events
   
    auto square_colliders = world.query<const Collider, const SquareCollider, const Transform>("SquareColliders");

    world.system("Collision")
        .kind(flecs::PreUpdate)
        .run([square_colliders = std::move(square_colliders)](flecs::iter&) {
