...
public:
    ...
    Pimpl(const services::RoomManager& room_manager, dxgl::UboBindingManager& ubo_manager) {
        ...
        room_manager.room_removed_signal.connect<&Pimpl::OnRoomRemoved>(this);     
...
void RoomRenderer::PimplDeleter::operator()(Pimpl* p) const {
    delete p;
}
...
RoomRenderer::RoomRenderer(RoomRenderer&& move) noexcept = default;
RoomRenderer::~RoomRenderer() = default;
RoomRenderer& RoomRenderer::operator=(RoomRenderer&& move) noexcept = default;

void RoomRenderer::Render(DrawQueues& draws) const {
    ...
    for (auto& [room_id, draw] : m_pimpl->cached_draws) {
        if (!draw.has_value()) {
            draw = m_pimpl->BuildDraw(*m_pimpl->room_manager->GetRoom(room_id));
        }
        
        draw->uniform_applicator = [color = colors[color_index], this](dxgl::ProgramRef program) {
            dxgl::Uniform::Set(*program, "color", color);
            
            const float ombre_width = m_pimpl->room_manager->GetTileGrid().GetTileWorldSize().x / 10.f;
