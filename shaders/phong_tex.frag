...
void main() {
    ...
    // Emission
    vec2 emission_tex_coords = vec2(frag_tex_coords.s, frag_tex_coords.t + time);
    vec3 emission_light = vec3(texture(material.emission_map, emission_tex_coords)) * abs(sin(time));
